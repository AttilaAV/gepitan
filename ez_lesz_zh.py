...
#2. feladat
grouped_by_target = df.groupby(by=10);  # grouping by target
# Exporting a target group from the grouped dataframe


# Basic descriptive stats
mean_by_target = grouped_by_target.mean();  # mean
std_by_target = grouped_by_target.std();  # standard deviation
corr_by_target = grouped_by_target.corr();  #  correlations
desc_stat_by_target = grouped_by_target.describe();  # desc stat with quantiles
...
plt.figure(1);
pd.plotting.scatter_matrix(df[[0,1,2,3]]);
plt.show();
